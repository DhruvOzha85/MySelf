          ...
          {sortedProjects.map((project, index) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -8 }}
                className={`group bg-card rounded-3xl border overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 flex ${
                  isFeatured 
                    ? "md:col-span-2 lg:col-span-12 flex-col lg:flex-row border-primary/20 hover:border-primary/50" 
                    : "col-span-1 lg:col-span-6 flex-col border-border hover:border-primary/30"
                }`}
              >
                {/* Visuals (Image Side) */}
                <div className={`relative overflow-hidden bg-secondary/30 flex items-center justify-center ${
                  ...
                  <img
                    src={project.image}
                    alt={project.title}
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-[cubic-bezier(0.33,1,0.68,1)]"
                  />
                </div>
